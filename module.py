...
def discriminator(image, options, reuse=False, name="discriminator"):
    ...
    with tf.variable_scope(name):
        ...
        d1 = tf.image.resize_images(e8,[tf.shape(e8)[1]*2,tf.shape(e8)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d1 = conv2d(d1, options.gf_dim*8, ks=3, s=1, name='g_d1')
        d1 = tf.concat([tf.nn.dropout(instance_norm(d1, 'g_bn_d1'), 0.5), e7], 3)
        # d1 is (2 x 2 x self.gf_dim*8*2)

        d2 = tf.image.resize_images(d1,[tf.shape(d1)[1]*2,tf.shape(d1)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d2 = conv2d(d2, options.gf_dim*8, ks=3, s=1, name='g_d2')
        d2 = tf.concat([tf.nn.dropout(instance_norm(d2, 'g_bn_d2'), 0.5), e6], 3)
        # d2 is (4 x 4 x self.gf_dim*8*2)

        d3 = tf.image.resize_images(d2,[tf.shape(d2)[1]*2,tf.shape(d2)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d3 = conv2d(d3, options.gf_dim*8, ks=3, s=1, name='g_d3')
        d3 = tf.concat([tf.nn.dropout(instance_norm(d3, 'g_bn_d3'), 0.5), e5], 3)
        # d3 is (8 x 8 x self.gf_dim*8*2)

        d4 = tf.image.resize_images(d3,[tf.shape(d3)[1]*2,tf.shape(d3)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d4 = conv2d(d4, options.gf_dim*8, ks=3, s=1, name='g_d4')
        d4 = tf.concat([instance_norm(d4, 'g_bn_d4'), e4], 3)
        # d4 is (16 x 16 x self.gf_dim*8*2)

        d5 = tf.image.resize_images(d4,[tf.shape(d4)[1]*2,tf.shape(d4)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d5 = conv2d(d5, options.gf_dim*4, ks=3, s=1, name='g_d5')
        d5 = tf.concat([instance_norm(d5, 'g_bn_d5'), e3], 3)
        # d5 is (32 x 32 x self.gf_dim*4*2)

        d6 = tf.image.resize_images(d5,[tf.shape(d5)[1]*2,tf.shape(d5)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d6 = conv2d(d6, options.gf_dim*2, ks=3, s=1, name='g_d6')
        d6 = tf.concat([instance_norm(d6, 'g_bn_d6'), e2], 3)
        # d6 is (64 x 64 x self.gf_dim*2*2)

        d7 = tf.image.resize_images(d6,[tf.shape(d6)[1]*2,tf.shape(d6)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d7 = conv2d(d7, options.gf_dim, ks=3, s=1, name='g_d7')
        d7 = tf.concat([instance_norm(d7, 'g_bn_d7'), e1], 3)
        # d7 is (128 x 128 x self.gf_dim*1*2)

        d8 = tf.image.resize_images(d7,[tf.shape(d7)[1]*2,tf.shape(d7)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d8 = conv2d(d8, 34, ks=3, s=1, name='g_d8')
        # d8 is (256 x 256 x 1)
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        d1 = tf.image.resize_images(r9,[tf.shape(r9)[1]*2,tf.shape(r9)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d1 = conv2d(d1, options.gf_dim*2, ks=3, s=1, name='g_d1_dc')
        d1 = tf.nn.relu(instance_norm(d1, 'g_d1_bn'))
        d2 = tf.image.resize_images(d1,[tf.shape(d1)[1]*2,tf.shape(d1)[2]*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        d2 = conv2d(d2, options.gf_dim, ks=3, s=1, name ='g_d2_dc')
